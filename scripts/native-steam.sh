...
if [ -f "$(which apt)" ]; then
	dist=deb
	x64dll=/usr/lib/x86_64-linux-gnu/wine/wineasio64.dll
	x64so=/usr/lib/x86_64-linux-gnu/wine/wineasio64.dll.so
...
## TODO read proton path from steam config
...
case $dist in
	"deb")
		...
		# https://kx.studio/Repositories 2025-07-12
		wget https://launchpad.net/~kxstudio-debian/+archive/kxstudio/+files/kxstudio-repos_11.2.0_all.deb -P /tmp
		sudo dpkg -i /tmp/kxstudio-repos_11.2.0_all.deb
...
cp "$x64dll" "$runner/lib/wine/x86_64-windows/wineasio64.dll"
cp "$x64so" "$runner/lib/wine/x86_64-unix/wineasio64.dll.so"

mv $prefix "$prefix.bak" # backs up an already existing prefix, makes place for a clean new one
...
echo "================================"
echo
## TODO this didn't pause execution
read -p "Press enter to open the game"
xdg-open steam://rungameid/221680 #run game

echo
read -p "Press enter after closing the game"

WINEPREFIX="$prefix" wineasio-register # register wineasio in prefix
